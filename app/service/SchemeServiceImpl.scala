...
class SchemeServiceImpl @Inject()(schemeConnector: SchemeConnector, barsConnector: BarsConnector,
  ...
  private[service] def transformJsonToModel[A<:Declaration](json: JsValue, readsDeclaration: Reads[A]): Either[BadRequestException, PensionsScheme] = {
    ...
    val result = for {
      ...
      establishers <- json.validate[EstablisherDetails](readsEstablisherDetails(fs.get(Toggles.isSeparateRefCollectionEnabled)))
      trustees <- json.validate[TrusteeDetails](readsTrusteeDetails(fs.get(Toggles.isSeparateRefCollectionEnabled)))
