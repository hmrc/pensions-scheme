...
class SchemeSubscriptionCacheRepositorySpec extends AnyWordSpec with MockitoSugar with Matchers with MongoEmbedDatabase with BeforeAndAfter with
...
object SchemeSubscriptionCacheRepositorySpec extends AnyWordSpec with MockitoSugar {
  ...
  private def mongoCollectionDrop(): Void = Await
    .result(schemeSubscriptionCacheRepository.collection.drop().toFuture(), Duration.Inf)

  def schemeSubscriptionCacheRepository: SchemeSubscriptionCacheRepository =
    new SchemeSubscriptionCacheRepository(mockAppConfig, mongoComponent)
}