...
class AddressReadsSpec extends CommonAddressTest {
  "A JSON Payload with an address" should {
    "Map correctly to an Address type" when {

      val correspondenceAddress = Json.obj("line1" -> JsString("line1"), "line2" -> JsString("line2"), "line3" -> JsString("line3"), "line4" -> JsString("line4"),
        "postalCode" -> JsString("NE1"), "countryCode" -> JsString("GB"))

      behave like correspondenceAddressDetails(correspondenceAddress)
...
trait CommonAddressTest extends WordSpec with MustMatchers with OptionValues with Samples {
