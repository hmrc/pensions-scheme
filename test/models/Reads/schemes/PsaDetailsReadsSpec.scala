...
class PsaDetailsReadsSpec extends WordSpec with MustMatchers with OptionValues {
  ...
  "A JSON payload containing psa details" should {
    "read into a valid PSA Details object " when {
      ...
      "we have a psa id" in {
        input.as(PsaDetails.apiReads).id mustBe (input \ "psaid").as[String]
      }

      "we have an organisation name" in {
        input.as(PsaDetails.apiReads).organisationOrPartnershipName.value mustBe (input \ "organizationOrPartnershipName").as[String]
      }
      ...
      "we have a firstName" in {
        input.as(PsaDetails.apiReads).individual.value.firstName.value mustBe (input \ "firstName").as[String]
      }
      ...
      "we have a middleName" in {
        input.as(PsaDetails.apiReads).individual.value.middleName.value mustBe (input \ "middleName").as[String]
      }
      ...
      "we have a lastName" in {
        input.as(PsaDetails.apiReads).individual.value.lastName.value mustBe (input \ "lastName").as[String]
      }
