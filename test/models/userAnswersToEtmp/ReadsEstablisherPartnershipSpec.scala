    ...
    "must be read from valid data" in {
      forAll(partnershipGenerator) { json =>
        ...
        if ((json \ "hasUtr").as[Boolean]) {
          ...
          transformedEstablisher.noUtrReason mustBe None
        } else {
          transformedEstablisher.utr mustBe None
