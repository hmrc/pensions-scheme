...
import models.{Address, CompanyTrustee, PreviousAddressDetails}
...
class ReadsTrusteeCompanySpec extends WordSpec with MustMatchers with OptionValues {
  ...
  val companiesGen: Gen[JsArray] =
    for {
      ...
      company2 <- companyGenerator(true)
  ...
  private def companyGenerator(isDeleted: Boolean = false): Gen[JsObject] =
    for {
      ...
      "trusteesCompanyAddressYears" -> addressYears,
  ...
  "A Json payload containing trustee company" should {
    ...
    "read company name correctly" in {
        forAll(companiesGen) { json =>
          val model = json.as[Seq[CompanyTrustee]](readsTrusteeCompanies)
