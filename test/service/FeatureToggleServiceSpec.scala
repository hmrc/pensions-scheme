...
import models.FeatureToggleName.{DummyToggle, SchemeRegistration}
...
class FeatureToggleServiceSpec
  ...
  "When getAll is called returns all of the toggles from the repo" in {
    OUT.getAll.futureValue mustBe Seq(
      ...
      Disabled(SchemeRegistration)
    )
  }

  "When a toggle doesn't exist in the repo, return default" in {
    when(adminDataRepository.getFeatureToggles).thenReturn(Future.successful(Seq.empty))
    OUT.get(SchemeRegistration).futureValue mustBe Disabled(SchemeRegistration)
  }

  "When a toggle exists in the repo, override default" in {
    when(adminDataRepository.getFeatureToggles).thenReturn(Future.successful(Seq(Enabled(SchemeRegistration))))
    OUT.get(SchemeRegistration).futureValue mustBe Enabled(SchemeRegistration)
  }
